...
from Sources.utils import RolloutBuffer
...
class SAC_continuous(Algorithm):
    ...
    def step(self, env, state, ep_len):
        ...
        for idx in range(self.num_envs):
            ...
            self.tmp_buffer[idx].append((state[idx], action[idx], reward[idx] * self.reward_factor, mask, log_pi[idx], next_state[idx]))
            ...
            if (done[idx]):
                ...
                for (tmp_state,tmp_action,tmp_reward,tmp_mask,tmp_log_pi,tmp_next_state) in self.tmp_buffer[idx]:
                    self.buffer.append(tmp_state, tmp_action, tmp_reward,self.tmp_return_reward[idx],
                        tmp_mask, tmp_log_pi, tmp_next_state)
                self.tmp_buffer[idx] = []
    ...
    def update(self, log_info):
        ...
        states, actions, env_rewards,env_total_rewards, dones, log_pis, next_states =\
